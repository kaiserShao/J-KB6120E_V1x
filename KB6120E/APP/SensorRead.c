		...
		do
		{
			switch ( option )
			{
				...
				default:
					...
					Lputs( 0x0600u, sbuffer );
					sprintf( sbuffer, "[%c]%5u", ( OutState[PumpSelect] ? '+' : '-' ), OutValue[PumpSelect] );
					Lputs( 0x0608u, sbuffer );
					break;
				case opt_AIR:
					...
					Lputs( 0x0400u, sbuffer );
					sprintf( sbuffer, "环 境:%6.4f", _CV_DS18B20_Temp( SensorRemote.Te ));
					Lputs( 0x0600u, sbuffer );
					sprintf( sbuffer, "[%c]", ( OutState[PumpSelect] ? '+' : '-' ));
					Lputs( 0x060Cu, sbuffer );
					break;
