...
void  SD_File_Creat( void )
{
	...
	else if( strncmp( (char*)sdinit, (char*)bufread, 15 ) ) //检测文件是否正常 返回零为正常
	{
	...
	if( ! ByteLoad( "\\SD_TSP\\TSP.TXT", 0, (uint8_t *) bufread, 15 ) )
	{
		...
		SampleSet[Q_TSP].FileNum = 0;
		SetFlag = TRUE;
	}
	else if( strncmp( (char*)sdinit, (char*)bufread, 15 ) ) 
	{
		Byte_CREAT_WRITE_PATH( "\\SD_TSP\\TSP.TXT", sdinit,16 );
		ByteFill( "\\SD_TSP\\TSP.TXT", 16, (FilePageSize_TSP - sizeof ( sdinit ) + FilePageSize_TSP ));
		SampleSet[Q_TSP].FileNum = 0;
...
void USB_File_Creat(void)
{
	...
	else if( strncmp( (char*)USBinit, (char*)bufread, 15 ) ) //检测文件是否正常 返回零为正常
	{
